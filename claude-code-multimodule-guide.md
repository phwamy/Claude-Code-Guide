...
|----------|---------|-------------------|--------|-------------------------------|--------------|
| PRD.md | Product vision, use cases, scope, success criteria, out-of-scope | Pre-build · You + Claude.ai via the `project-scoping` skill | Product owner, Orchestrator agent | When scope, use cases, or success criteria change. You update with Claude.ai, then re-share with Orchestrator. Version it (v1.1, v1.2). | ✍️ No built-in. Use the `project-scoping` skill — it interviews you across 3 rounds and runs a Use Case Review Loop that challenges every use case on value, effort, and phase-fit before locking scope. |
| ROADMAP.md | Deferred use cases from scoping, organized by phase (Phase 2 / Future). Preserves ideas deliberately excluded from Phase 1 so they aren't lost or re-litigated. Each item includes a deferral reason and a suggested promotion trigger. | Pre-build · You + Claude.ai during `project-scoping` — generated as a byproduct of the Use Case Review Loop, immediately after PRD.md is locked | Product owner only. **Never shared with Orchestrator or builder agents** — deferred items must not influence Phase 1 architecture. | When a new use case is deferred mid-build, append it. When Phase 1 ships, review Phase 2 items against real usage data before promoting any to a new PRD. Never edit in-place — promote items by opening a new scoping session. | ✍️ No built-in. Generated automatically by the `project-scoping` skill as part of the Use Case Review Loop. |
| ASSUMPTIONS.md | All assumptions made, risk level, status (holding / under review / invalidated) | Pre-build · You + Claude.ai during `project-scoping` — surface what you're taking for granted | Product owner, Orchestrator | When any assumption is challenged or invalidated during build. Agents append new assumptions; you review and resolve. | ✍️ No built-in. Generated automatically by the `project-scoping` skill. Instruct agents in CLAUDE.md: "Append to ASSUMPTIONS.md when you make a judgment call." |
| ARCHITECTURE.md | System design, module map, data flow, tech stack decisions, integration points | Pre-build · You + Claude.ai via the `solution-design` skill, then validated in Claude Code Phase 0 | Orchestrator agent, all builder agents | When a module boundary, integration pattern, or stack decision changes. Orchestrator proposes; you approve. | ✍️ No built-in. Use the `solution-design` skill — it takes PRD.md + ASSUMPTIONS.md as input, interviews you on technical constraints, and generates this doc. |
| SCENARIOS.md | End-to-end user journeys and business scenarios that cut across multiple modules. Defines "happy path" and key edge cases as narrative flows, not test code. Used to validate that module boundaries and interfaces are correct before build. | Pre-build · You + Claude.ai via the `solution-design` skill, after ARCHITECTURE.md is stable — derived from PRD use cases | Product owner, Orchestrator agent, all builder agents | When PRD use cases change or new cross-module flows are discovered during build. Orchestrator flags if a scenario can no longer be satisfied. | ✍️ No built-in. Generated automatically by the `solution-design` skill alongside ARCHITECTURE.md. |
